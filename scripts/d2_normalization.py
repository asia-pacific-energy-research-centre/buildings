...
initial_df = pd.read_csv(config.root_dir + '/output_data/d1_end_use_split_by_fuel/fuel_split_end_use.csv')
...
output_dir_csv = config.root_dir + '/output_data/d2_normalization/'
...
output_dir_fig = config.root_dir + '/plotting_output/d2_normalization/'
if not os.path.exists(output_dir_fig):
    os.makedirs(output_dir_fig)

concat_final = concat_final[concat_final['fuel_amount'] != 0].copy()
# %%

plots = concat_final.copy()

plot_res = plots[plots['sub2sectors'] == '16_01_02_residential']
fig = px.area(plot_res, x='year', y='fuel_amount', color='fuel', facet_col='economy', facet_col_wrap=7)
fig.update_yaxes(matches=None, showticklabels=True)
fig.show()
fig.write_html(output_dir_fig + 'res_fuels_normalized.html')

plot_srv = plots[plots['sub2sectors'] == '16_01_01_commercial_and_public_services']
fig = px.area(plot_srv, x='year', y='fuel_amount', color='fuel', facet_col='economy', facet_col_wrap=7)
fig.update_yaxes(matches=None, showticklabels=True)
fig.show()
fig.write_html(output_dir_fig + 'srv_fuels_normalized.html')
# %%
...
ratio_normalized.to_csv(output_dir_csv + 'ratio_for_d4.csv', index=False)
