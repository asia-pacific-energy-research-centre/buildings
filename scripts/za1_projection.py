# FINAL OUTPUT IS END USE PROJECTION TO 2100 FOR ALL APEX ECONOMIES
# BASED ON SOME AVERAGE DATA FOR NON-IEA ECONOMIES, RATIO OF END USES COMPRISING TOTAL ENERGY THEN MULTIPLIED THROUGH PROJECTED ENERGY
...
# CALCULATING THE PERCENT OF TOTAL THAT END USES MAKE UP. MIGHT NOT SUM TO 1, SO NEED TO NORMALIZE
# I THINK THIS IS ALREADY SOLVED BY NORMALIZING BELOW
###########################################################################################################################################
###########################################################################################################################################
###########################################################################################################################################
###########################################################################################################################################
...
energy_by_end_use_other = eei.loc[(eei['measure'] == 'energy') & (eei.end_use.isin(END_USES)) & (eei.fuel=='all')].copy()
...
energy_by_end_use_other['ratio'] = (energy_by_end_use_other['value_end_use']/energy_by_end_use_other['value_total'])
#########
#plot the percentage of the total that each of these end uses make up

# output_dir = config.root_dir + '/plotting_output/analysis/END_USE_SIX/'
# if not os.path.exists(output_dir):
#     os.makedirs(output_dir)
fig = px.line(energy_by_end_use_other, x='year', y='ratio', color='end_use', facet_col='economy', facet_col_wrap=3, line_dash='sector')
# fig.write_html(output_dir + 'energy_by_end_use_others.html')

# energy_by_end_use_other.to_csv(output_dir + 'energy_by_end_use_ratio.csv')
#########
#%%
...
output_dir_csv = config.root_dir + '/output_data/a1_projection/'
if not os.path.exists(output_dir_csv):
    os.makedirs(output_dir_csv)
# to csv
new_scaled_projection_df_merge.to_csv(output_dir_csv + 'projection_w_end_use_scaled.csv', index=False)


output_dir_fig = config.root_dir + '/plotting_output/a1_projection/'
if not os.path.exists(output_dir_fig):
    os.makedirs(output_dir_fig)
#plot
fig.write_html(output_dir_fig + 'energy_use_by_end_use_sector_scaled.html')





# THIS ENERGY PROJECTION IS WHAT CAN BE USED TO THEM USE THE FUEL SPLIT
