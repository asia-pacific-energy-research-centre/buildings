...
# %%
from b1_end_use_adjustment_function import traj

# %%
end_use_trajectory_interim = pd.read_csv(config.root_dir + '/output_data/a1_projection/projection_w_end_use_scaled.csv')
...
# traj(economy = '01_AUS', fuels = '01_coal', proj_start_year = 2022, 
...
for economy in economy_list['economy_code']:
    filenames = glob.glob(config.root_dir + '/output_data/b2_end_use_adjustment_apply/{}/*.csv'.format(economy))
    for i in filenames:
...
output_dir_csv = config.root_dir + '/output_data/b2_end_use_adjustment_apply/'
if not os.path.exists(output_dir_csv):
    os.makedirs(output_dir_csv)

output_dir_fig = config.root_dir + '/plotting_output/b2_end_use_adjustment_apply/'
if not os.path.exists(output_dir_fig):
    os.makedirs(output_dir_fig)

traj_overwrite_df.to_csv(output_dir_csv + 'adjusted_enduses.csv', index=False)
...
output_dir_fig_line = config.root_dir + '/plotting_output/b2_end_use_adjustment_apply/by_economy/line_plots/'
if not os.path.exists(output_dir_fig_line):
    os.makedirs(output_dir_fig_line)

output_dir_fig_area = config.root_dir + '/plotting_output/b2_end_use_adjustment_apply/by_economy/area_plots/'
